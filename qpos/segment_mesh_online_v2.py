...
from qpos.v2.geometric_instances import read_ply_scannet, get_vertex_root, merge_mean_cov_est, \
    check_energy
...
def incremental_frame_segmentation(verts, normals, faces, verts_mask, segdata, params):
...
class IncrementalSegmenter:
    def __init__(self, verts, faces, normals, colors, trajectory, segmentation_params,
                 K, segmentation_processor, path_to_depth, skip=10, depth_diff_threshold=0.1):
        """
    ...
    def move_forward(self, vis):
        ...
        if self.check_pose_valid(T):
            ...
            incremental_frame_segmentation(self.mesh_vertices, self.mesh_normals, self.mesh_faces,
                                           mask_vertices, self, self.segmentation_params)
            is_finish = time.time()
...
def process_sequence_with_segmenter(scene_dir, segmentation_processor, path_to_mesh, path_to_depth, path_to_intri, expected_segment_size=0.1,
                                    small_segment_size=0.02, width=640, height=480, skip=10, depth_diff_threshold=0.1):
    ...
    # v2 parameters
    segmentation_params = [small_segment_size, expected_segment_size]
...
class SegmentationLogger:
    def __init__(self, debug_output_path, path_to_mesh, mapping_every=10):
        ...
        self.plydata_mask = deepcopy(plydata)
        ...
        if (current_frame_id % self.mapping_every == 0) or is_last_frame:
            ...
            if self.is_save_all:
                ...
                if np.max(seg_ids) > self.max_seg_id:
                    self.colour_map_seg = colour_map_using_hash_ids(seg_ids)
